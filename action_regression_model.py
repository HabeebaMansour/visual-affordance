...
class ActionRegressionDataset(Dataset):
    ...
    def __getitem__(self, idx: int) -> Dict[str, torch.Tensor]:
        ...
        input_img = torch.permute(data['rgb'], (2, 0, 1)) / 255

        angle_norm=angle/180
        center_point=center_point/127

        target=np.array([center_point[0], center_point[1], angle_norm])
...
def recover_action(
    ...
    angle=action[2]*180
    center_point_x=action[0]*127
    center_point_y=action[1]*127

    coord=(center_point_x.astype(int), center_point_y.astype(int))
    # ===============================================================================
    return coord, angle.astype(int)
...
class ActionRegressionModel(nn.Module):
    ...
    def predict_grasp(self, rgb_obs: np.ndarray
        ...
        stacked_rgb= r.reshape(1, rgb_obs.shape[0],rgb_obs.shape[1],rgb_obs.shape[2])
        ...
        action= affordance_map[0].detach().numpy()
        
        coord, angle = recover_action(action)
        # ===============================================================================
        # visualization

        vis_img = self.visualize(rgb_obs.transpose((2,0,1)), action)
