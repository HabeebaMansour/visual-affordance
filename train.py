...
def get_center_angle(
    ...
    center_coord = np.mean([left_coord, right_coord], axis=0)

    # Calculate the direction vector of the gripper
    direction = np.subtract(right_coord, left_coord)
...
class AugmentedDataset(Dataset):
    ...
    def __getitem__(self, idx: int
        ...
        #1:get_finger_points(center_point, angle)
        #2:KeyPointsonImage
        #3:self.aug_pipeline
        #4:get_center_angle
        ...
        rgb_aug, kps_aug = self.aug_pipeline(image=rgb.numpy(), keypoints=kps)

        left_coord_aug=kps_aug.keypoints[0]
        right_coord_aug=kps_aug.keypoints[1]
        

        center_coord_aug, angle_aug=get_center_angle((left_coord_aug.x, left_coord_aug.y), (right_coord_aug.x,right_coord_aug.y ))

        data_torch = {
            'rgb': torch.from_numpy(rgb_aug).float(),
